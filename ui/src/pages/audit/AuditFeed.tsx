...
const ACTION_DOMAINS: { value: string; label: string }[] = [
  { value: ALL, label: "audit.actionDomains.all" },
  { value: "issue.", label: "audit.actionDomains.tasks" },
  { value: "agent.", label: "audit.actionDomains.agents" },
  { value: "heartbeat.", label: "audit.actionDomains.runs" },
  { value: "approval.", label: "audit.actionDomains.approvals" },
  { value: "project.", label: "audit.actionDomains.projects" },
  { value: "goal.", label: "audit.actionDomains.goals" },
  { value: "tool_gateway.", label: "audit.actionDomains.tools" },
  { value: "cost.", label: "audit.actionDomains.costs" },
  { value: "company.", label: "audit.actionDomains.company" },
];

/** Entity types offered in the filter (server does an exact match). */
const ENTITY_TYPES: { value: string; label: string }[] = [
  { value: ALL, label: "audit.entityTypes.all" },
  { value: "issue", label: "audit.entityTypes.task" },
  { value: "agent", label: "audit.entityTypes.agent" },
  { value: "project", label: "audit.entityTypes.project" },
  { value: "goal", label: "audit.entityTypes.goal" },
  { value: "company", label: "audit.entityTypes.company" },
];
...
function AuditActor({
  ...
  if (record.actorType === "user" && record.actorId) {
    ...
    return (
      <Identity
        name={profile?.label ?? t("audit.user")}
  ...
  const label =
    record.actorType === "plugin"
      ? t("audit.actorTypes.plugin")
      : record.actorType === "agent"
        ? t("audit.actorTypes.agent")
        : record.actorType === "user"
          ? t("audit.actorTypes.user")
          : t("audit.actorTypes.system");
...
function AuditEntityNode({ record }: { record: AuditActionRecord }) {
  ...
  if (issueRef) {
    return (
      <Link to={`/issues/${issueRef}`} className="font-medium text-primary hover:underline">
        {issue?.identifier ? `${issue.identifier}${issue.title ? ` · ${issue.title}` : ""}` : t("audit.theTask")}
      </Link>
    );
  }
  if (document) {
  ...
  const entityLabel = t(`audit.entityTypes.${record.entityType}`, { defaultValue: record.entityType });
  return <span className="text-muted-foreground">{entityLabel}</span>;
}
...
function AuditRow({
  ...
  const responsibleLabel = responsible?.label ?? (record.responsibleUserId ? t("audit.aUser") : null);
        ...
        <div className="min-w-0 space-y-1">
          ...
          {documentKey ? (
            <p className="text-xs text-muted-foreground">
              {t("audit.document")} <span className="font-mono text-(length:--text-micro)">{documentKey}</span>
            </p>
          ) : null}
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            {showOnBehalf && responsibleLabel ? (
              <span className="inline-flex items-center rounded-full bg-muted px-2 py-0.5">
                {t("audit.onBehalfOf", { responsible: responsibleLabel })}
              </span>
            ) : null}
  ...
  return (
    <Card>
      <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-foreground">{t("audit.enterpriseTitle")}</p>
          <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
            {t("audit.enterpriseDescription")} {t("audit.enterpriseAsk")} {" "}
            <span className="font-mono text-(length:--text-micro)">audit:view_agent_actions</span>{" "}
            {t("audit.enterprisePermission")}
          </p>
        </div>
      </CardContent>
    </Card>
...
export function AuditFeed({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      pushToast({ title: t("audit.exported"), body: t("audit.downloadStarted"), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("audit.exportFailed"),
        body: error instanceof Error ? error.message : t("audit.exportError"),
        tone: "error",
      });
    } finally {
      setExporting(false);
    ...
    <div className="space-y-4">
      {!hideHeader ? (
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-semibold text-foreground">{t("audit.activity")}</h1>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              {resolvedMode === "agents"
                ? t("audit.agentDescription")
                : t("audit.companyDescription")}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {showModeToggle ? (
        <Tabs value={resolvedMode} onValueChange={(value) => onModeChange?.(value as AuditFeedMode)}>
          <TabsList aria-label={t("audit.activityScope")}>
            <TabsTrigger value="all">{t("audit.allActivity")}</TabsTrigger>
            <TabsTrigger value="agents">{t("audit.agentActions")}</TabsTrigger>
          </TabsList>
        </Tabs>
      ) : null}
      ...
      {canUseAdvancedControls ? (
        <div className="flex flex-wrap items-center gap-2">
          {!lockedAgentId ? (
            <Select value={agent} onValueChange={setAgent}>
              <SelectTrigger className="w-40">
                <SelectValue placeholder={t("audit.agent")} />
              </SelectTrigger>
              <SelectContent>
          ...
          <Select value={responsibleUser} onValueChange={setResponsibleUser}>
            ...
            <SelectValue placeholder={t("audit.responsibleUser")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL}>{t("audit.allResponsibleUsers")}</SelectItem>
      ...
      {recoveringFromAccessDowngrade || fallingBackToAllActivity ? (
        <Card>
          <CardContent className="py-14 text-center text-sm text-muted-foreground">
            {t("audit.refreshingAccess")}
          </CardContent>
        </Card>
      ) : feed.isLoading ? (
        <Card>
          <CardContent className="py-14 text-center text-sm text-muted-foreground">{t("audit.loading")}</CardContent>
        </Card>
      ) : feed.error ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
            <p className="text-sm text-muted-foreground">
              {feed.error instanceof Error ? feed.error.message : t("audit.loadError")}
            </p>
          ...
          <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">
                {hasActiveFilters ? t("audit.noActionsMatch") : t("audit.empty")}
              </p>
              <p className="mt-1 max-w-md text-sm text-muted-foreground">
                {hasActiveFilters
                  ? t("audit.widenFilters")
                  : resolvedMode === "agents"
                    ? t("audit.agentEmptyHint")
                    : t("audit.companyEmptyHint")}
              </p>
            </div>
            {hasActiveFilters ? (
      ...
      {feed.hasNextPage ? (
        <div className="flex justify-center">
          ...
          >
            {feed.isFetchingNextPage ? t("audit.loading") : t("audit.loadMore")}
          </Button>
        </div>
      ) : null}
