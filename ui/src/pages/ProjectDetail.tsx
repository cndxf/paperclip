  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("project.targetDateLabel")}</span>
    ...
    >
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("project.changeIconColor")}
        >
      ...
      <PopoverContent className="w-72 p-3" align="start">
        {/* Icon search + grid */}
        <p className="text-xs font-medium text-muted-foreground mb-2">{t("project.icon")}</p>
        <Input
        ...
        <div className="grid grid-cols-7 gap-1 max-h-40 overflow-y-auto">
          ...
          {filteredIcons.length === 0 && (
            <p className="col-span-7 text-xs text-muted-foreground text-center py-2">{t("project.noIconsMatch")}</p>
          )}
        </div>

        {/* Color swatches */}
        <div className="mt-3 border-t border-border pt-3">
          <p className="text-xs font-medium text-muted-foreground mb-2">{t("project.color")}</p>
          <div className="grid grid-cols-5 gap-1.5">
            ...
            <button
              ...
              title={t("project.neutralDefault")}
            >
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("project.projects");
      if (archived) {
    ...
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("project.failedArchive") : t("project.failedUnarchive"),
        tone: "error",
      });
    },
  });
  ...
  const uploadImage = useMutation({
    mutationFn: async (file: File) => {
      if (!resolvedCompanyId) throw new Error(t("tasks.selectCompany"));
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("project.projects"), href: "/projects" },
      { label: project?.name ?? routeProjectRef ?? t("project.projects") },
    ]);
