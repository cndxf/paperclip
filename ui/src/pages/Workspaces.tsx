  ...
  return (
    <div className="space-y-6">
      ...
      <SummarySlotCard
        ...
        title={t("workspace.summaryTitle")}
        description={t("workspace.summaryDescription")}
      />

      {groups.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("workspace.emptyActivity")}</p>
      ) : (
            ...
            <section key={group.projectId} className="space-y-3">
              <div className="flex flex-wrap items-end justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t(group.summaries.length === 1 ? "workspace.count_one" : "workspace.count_other", { count: group.summaries.length })}
                </span>
              </div>
          ...
          {overviewQuery.hasNextPage ? (
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
              <p className="text-sm text-muted-foreground">
                {t("workspace.showing", { shown: overviewItems.length, total: totalWorkspaceCount })}
              </p>
              <Button
                type="button"
                variant="outline"
              ...
              >
                {overviewQuery.isFetchingNextPage ? t("workspace.loading") : t("workspace.loadMore")}
              </Button>
            </div>
          ) : null}
        </div>
