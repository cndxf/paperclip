  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.financeLedger", { defaultValue: "财务台账" })}</CardTitle>
        <CardDescription>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <MetricTile
          label={t("costs.estimated", { defaultValue: "预估" })}
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={`${formatTokens(inferenceTokenTotal)} 个令牌，来自请求事件`}
              icon={DollarSign}
            />
            <MetricTile
              label="Budget"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ...
                    : t("costs.noMonthlyCap", { defaultValue: "未设置月度上限" })
              }
              icon={Coins}
            />
            <MetricTile
              ...
              subtitle={`借记 ${formatCents(financeData?.summary.debitCents ?? 0)} · 贷记 ${formatCents(financeData?.summary.creditCents ?? 0)}`}
              icon={ReceiptText}
            />
            <MetricTile
              label="财务事件"
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`区间内预计 ${formatCents(financeData?.summary.estimatedDebitCents ?? 0)}`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.selectDates")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          已使用本月预算的 {spendData.summary.utilizationPercent}%。
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">按智能体统计</CardTitle>
                    <CardDescription>查看各智能体在所选时间段内的消耗。</CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-4">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsHint", { defaultValue: "暂无财务事件。账单或贷项产生后，账户级费用会显示在这里。" })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">预算控制</CardTitle>
                  <CardDescription>
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.noBudgetPolicies", { defaultValue: "暂无预算策略。请在智能体或项目详情页设置预算，或使用现有的公司月度预算控制。" })}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
        ...
        <TabsContent value="providers" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">请选择开始日期和结束日期以加载数据。</p>
          ) : (
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noPeriodEvents")}</p>
                  ) : (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">按计费方统计</CardTitle>
                      <CardDescription>按产生借记或贷记的主体分组显示账户级财务事件。</CardDescription>
                    </CardHeader>
