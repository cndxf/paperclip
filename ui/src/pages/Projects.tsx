...
const PROJECT_SORT_OPTIONS: Array<{ field: ProjectSortField; key: string }> = [
  { field: "name", key: "project.name" },
  { field: "updated", key: "project.updated" },
  { field: "created", key: "project.created" },
  { field: "targetDate", key: "project.targetDate" },
];
...
export function Projects() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("project.projects") }]);
  }, [setBreadcrumbs, t]);
  ...
  const sortLabel = t(PROJECT_SORT_OPTIONS.find((option) => option.field === sortField)?.key ?? "project.name");

  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={t("project.selectCompany")} />;
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="w-fit text-xs" title={t("project.sort")}>
              <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
              <span>{t("project.sort")}: {sortLabel}</span>
            </Button>
          </PopoverTrigger>
                ...
                >
                  ...
                  {sortField === option.field ? (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {sortDir === "asc" ? t("project.ascending") : t("project.descending")}
                    </span>
                  ) : null}
                </button>
              ))}
      ...
      {projects.length > 0 && (
        <div className="space-y-6">
          {([
            [t("project.mine"), groupedProjects.mine],
            [t("project.other"), groupedProjects.other],
              ...
              <section key={label} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t(sectionProjects.length === 1 ? "project.count_one" : "project.count_other", { count: sectionProjects.length })}
                  </span>
                </div>
                        ...
                        trailing={
                          <div className="flex items-center gap-3">
                            <span
                              ...
                              title={t((project.taskCount ?? 0) === 1 ? "project.taskCount_one" : "project.taskCount_other", { count: formatNumber(project.taskCount ?? 0) })}
                            >
                              {t((project.taskCount ?? 0) === 1 ? "project.taskCount_one" : "project.taskCount_other", { count: formatNumber(project.taskCount ?? 0) })}
                            </span>
                            {project.budget && (
