...
function translateText(value: string) {
  ...
  const wholeSentence: Array<[RegExp, string]> = [
    ...
    [/^Configuration未完成\s*·\s*(.+)$/i, "配置未完成 · $1"], [/^Configuration未完成$/i, "配置未完成"], [/^t is not defined$/i, "变量 t 未定义"],
  ...
  const phraseReplacements: Array<[string, string]> = [
    ...
    ["idle", "空闲"],
    ["[system]", "[系统]"],
    ["Monthly UTC 预算", "每月 UTC 预算"],
    ["No cap 已配置", "未设置预算上限"],
    ["Soft alert at 80%", "达到 80% 时提醒"],
    ["预算 (USD)", "预算（美元）"],
    ["Set 预算", "设置预算"],
    ["This is exactly the tool set Paperclip will accept for", "以下是 Paperclip 将接受的完整工具集合，适用于"],
    ["个人资料 and policy edits are reflected within ~5 秒.", "个人资料和策略修改会在约 5 秒内生效。"],
    ["The 智能体's prompt can narrow this list but 无法展开—默认情况下不会阻止其他所有内容。", "智能体的提示词可以缩小此列表，但不能扩大列表；默认情况下不会阻止其他内容。"],
    ["已安装 apps load tools into seek's context on every 运行. Permitted-only apps do 未 add context cost.", "已安装的应用会在每次运行时将工具加载到智能体上下文中。仅获许可但未安装的应用不会增加上下文开销。"],
    ["Has access means the app is permitted. 已安装 means its tools are added to this 智能体's runtime context.", "拥有访问权限表示应用已获许可；已安装表示其工具会加入此智能体的运行时上下文。"],
    ["Why these tools?", "为什么是这些工具？"],
    ["Check access", "检查访问权限"],
    ["启用 POLICIES", "已启用的策略"],
    ["No enabled policy currently mutates this 智能体's allow list.", "当前没有已启用的策略会修改此智能体的允许列表。"],
    ["Every known tool this 智能体 could name is allowed.", "此智能体可能调用的所有已知工具均已允许。"],
    ["Fetched on demand 通过 the 运行-bound 智能体 API. Never written to the environment. The 智能体 reads them by alias through GET /agents/me/secrets.", "按需通过与运行绑定的智能体 API 获取。不会写入运行环境。智能体通过别名使用 GET /agents/me/secrets 读取这些密钥。"],
    ["Applied on next 运行", "将在下次运行时应用"],
    ["0 of 5 enabled", "已启用 0 / 5"],
    ["暂时没有skills enabled on this agent", "此智能体暂未启用任何技能"],
    ["分组 these 自动任务 into folders to keep things tidy.", "将这些自动任务分组到文件夹中，让页面更整洁。"],
    ["创建your first folder", "创建您的第一个文件夹"],
    ["draft", "草稿"],
    ["没有项目", "没有项目"],
    ["无默认值", "无默认智能体"],
    ["智能体 Never", "智能体：从不运行"],
    ["Organize r输出ines in this company.", "整理此公司的自动任务。"],
    ["stopped", "已停止"],
    ["暂时没有triggers", "暂时没有触发器"],
    ["新 trigger", "新建触发器"],
    ["还没有触发器。", "还没有触发器。"],
    ["添加a schedule", "添加计划"],
    ["已保存 适配器配置 affects the next 运行. 启用 runs keep the config they started with, and 配置变更 may start a new 的适配器会话.", "已保存。适配器配置会影响下一次运行；已启用的运行会继续使用启动时的配置，配置变更可能会开启新的适配器会话。"],
  ...
  broadCopy.push(
    [/^AWS 发现$/i, "AWS 发现"], [/^AWS 发现需要 ListSecrets 权限$/i, "AWS 发现需要 ListSecrets 权限"], [/^AWS 发现失败$/i, "AWS 发现失败"], [/^Beta 技能$/i, "测试版技能"],
    [/^(?:无效\s+)?email or password$/i, "邮箱或密码错误"], [/^Invalid email or password$/i, "邮箱或密码错误"],
    [/^What model would you like 您的第一个智能体 to use\? You can choose different models when creating additional agents\.?$/i, "您希望第一个智能体使用哪个模型？创建其他智能体时可以选择不同的模型。"],
    [/^Welcome! I'm (.+?), 您的第一个智能体 teammate on Paperclip\. I want to gather more context so I can come up with a plan and propose a team of agents to help execute it\. I'm putting together a few focused questions so we can settle on a concrete goal to tackle first\. Please give me one moment…?$/is, "您好！我是 $1，您在 Paperclip 中的首位智能体伙伴。我想先收集更多背景信息，以便制定计划，并提议一组智能体协助执行。我会先整理几个重点问题，帮助我们确定要优先处理的具体目标。请稍等片刻……"],
    [/^Welcome! I'm (.+?), 您的第一个智能体 teammate on Paperclip\.?$/i, "您好！我是 $1，您在 Paperclip 中的首位智能体伙伴。"],
    [/^I want to gather more context so I can come up with a plan and propose a team of agents to help execute it\. I'm putting together a few focused questions so we can settle on a concrete goal to tackle first\. Please give me one moment…?$/i, "我想先收集更多背景信息，以便制定计划，并提议一组智能体协助执行。我会先整理几个重点问题，帮助我们确定要优先处理的具体目标。请稍等片刻……"],
    [/^Message (.+?) — describe what you want done…?$/i, "给 $1 留言，描述您希望完成的内容……"],
    [/^No blockers$/i, "暂无阻塞项"], [/^正在搜索任务…?$/i, "正在搜索任务……"], [/^添加blocker$/i, "添加阻塞项"], [/^看板 USERS$/i, "看板用户"],
    [/^暂停 work…?$/i, "暂停工作……"], [/^Hide this task$/i, "隐藏此任务"],
    [/^Configuration未完成\s*·\s*(.+)$/i, "配置未完成 · $1"], [/^Configuration未完成$/i, "配置未完成"],
    [/^Paperclip stopped before dispatching the adapter because 必填 密钥\/env bindings are missing\.$/i, "Paperclip 在启动适配器前已停止，因为缺少必填的密钥或环境变量绑定。"],
    [/^Moving it to blocked so an 操作员 can bind the missing 密钥\(s\) before resuming\.$/i, "已将其移至“已阻塞”，操作员需要先绑定缺少的密钥，然后才能恢复运行。"],
    [/^Generating summary$/i, "正在生成摘要"], [/^Generate a concise status snapshot for this surface\.?$/i, "生成此页面的简要状态摘要。"],
    [/^Press Esc to close · Shortcuts are disabled in text fields$/i, "按 Esc 关闭；文本输入框中不会启用快捷键"], [/^Keyboard shortcuts$/i, "键盘快捷键"],
    [/^Folder$/i, "文件夹"], [/^Branch$/i, "分支"], [/^Path$/i, "路径"], [/^Service$/i, "服务"], [/^Integration smoke$/i, "集成冒烟测试"],
    [/^Requested by$/i, "请求人"], [/^Decision note\.?$/i, "决策备注。"], [/^Unsaved routine edits$/i, "自动任务有未保存的修改"], [/^Compare routine revisions$/i, "比较自动任务修订版本"], [/^Field$/i, "字段"], [/^Old value$/i, "旧值"], [/^New value$/i, "新值"], [/^Content$/i, "内容"], [/^Old$/i, "旧版"], [/^New$/i, "新版"], [/^Descriptions are identical\.?$/i, "两份描述完全相同。"],
    [/^New document$/i, "新建文档"], [/^Documents$/i, "文档"], [/^Out of date$/i, "已过期"], [/^Key$/i, "键"], [/^Coming soon$/i, "即将推出"], [/^Region$/i, "区域"], [/^Operation$/i, "操作"], [/^Provider$/i, "提供方"], [/^Vault context$/i, "密钥库上下文"], [/^Safe request\/error details$/i, "安全的请求 / 错误详情"], [/^Required IAM capability$/i, "所需 IAM 能力"], [/^Loading agents…$/i, "正在加载智能体……"], [/^Agent access$/i, "智能体访问权限"], [/^Provided by$/i, "提供者"],
    [/^No sub-goals\.?$/i, "暂无子目标。"], [/^No linked projects\.?$/i, "暂无关联项目。"], [/^Authenticated\. The environment has credentials now\.?$/i, "已完成身份验证，运行环境现在已有凭据。"], [/^Use manual model$/i, "使用手动指定的模型"], [/^Review →$/i, "审核 →"], [/^This page hit an error$/i, "此页面发生错误"], [/^Workspace plugin tab is not available\.?$/i, "工作区插件页签不可用。"], [/^Run routine$/i, "运行自动任务"], [/^Select an agent$/i, "选择智能体"], [/^Default agent required for this run\.?$/i, "运行此自动任务需要默认智能体。"], [/^True$/i, "是"], [/^False$/i, "否"], [/^Plan decomposition$/i, "计划拆分"], [/^Filter$/i, "筛选"], [/^Observed$/i, "已观测"], [/^Remaining$/i, "剩余"], [/^Enter a valid non-negative dollar amount\.?$/i, "请输入有效的非负金额。"],
    [/^Loading folder…$/i, "正在加载文件夹……"], [/^Load more from this folder$/i, "从此文件夹加载更多"], [/^Workspace recovery$/i, "工作区恢复"], [/^This team cannot be installed — the package manifest is invalid\.?$/i, "无法安装此团队，因为导出包清单无效。"], [/^required$/i, "必需"], [/^View imported agents →$/i, "查看已导入的智能体 →"],
    [/^What model would you like (.+?) to use\? You can choose different models when creating additional agents\.?$/i, "您希望 $1 使用哪个模型？创建其他智能体时可以选择不同的模型。"],
    [/^Welcome! I'm (.+?), (.+?) teammate on Paperclip\. I want to gather more context/i, "您好！我是 $1，您在 Paperclip 中的 $2。我想先收集更多背景信息"],
    [/^暂停 work…?$/i, "暂停工作……"], [/^Hide this task$/i, "隐藏此任务"], [/^No blockers$/i, "暂无阻塞项"], [/^看板 USERS$/i, "看板用户"],
    [/^Configuration未完成\s*·\s*(.+)$/i, "配置未完成 · $1"], [/^Configuration未完成$/i, "配置未完成"],
    [/^(.+?) Recovery needed$/i, "$1 需要恢复处理"],
    [/^(.+?) issue read marked (.+)$/i, "$1 已标记阅读事项 $2"],
    [/^(.+?) changed status to blocked on (.+)$/i, "$1 将 $2 的状态改为已阻塞"],
    [/^(.+?) agent instructions file已更新 (.+)$/i, "$1 更新了智能体指令文件 $2"],
    [/^(.+?) agent hire已创建 (.+)$/i, "$1 创建了智能体招聘记录 $2"],
    [/^agentDetail\.recentTasks$/i, "最近任务"],
    [/^issue\.read_marked$/i, "已标记阅读事项"], [/^issue\.updated$/i, "事项已更新"], [/^issue\.created$/i, "事项已创建"], [/^project\.created$/i, "项目已创建"], [/^智能体\.instructions_file_updated$/i, "智能体指令文件已更新"], [/^智能体\.hire_created$/i, "智能体招聘记录已创建"], [/^company\.created$/i, "公司已创建"],
    [/^(.+?) issue read marked (.+)$/i, "$1 已标记阅读事项 $2"], [/^(.+?) changed status to blocked on (.+)$/i, "$1 将 $2 的状态改为已阻塞"], [/^(.+?) 创建时间 (.+)$/i, "$1 创建了 $2"],
    [/^Recovery needed$/i, "需要恢复处理"], [/^chain stalled$/i, "处理链已停滞"],
    [/^任意人\s+except creator$/i, "除创建者外的任何人"],
    [/^新卡片s wait for a person on the board\. 智能体 are turned away\.?$/i, "新卡片等待看板用户处理，智能体不能回复。"],
    [/^New cards are open — the board or any agent can respond, including the one that asked\.?$/i, "新卡片对所有人开放，控制台或任何智能体都可以回复，包括发起请求的智能体。"],
    [/^选择\s*\{\{name\}\}$/i, "选择技能"],
    [/^如需在本地开发环境重新测试首次使用提示，请从当前实例的 instance_settings\.general JSON 记录中删除 feedbackDataSharingPreference，或将其恢复为 prompt。未设置和 prompt 都表示尚未选择默认值。$/i, "如需在本地开发环境重新测试首次使用提示，请从当前实例的 instance_settings.general JSON 记录中删除 feedbackDataSharingPreference，或将其恢复为 prompt。未设置和 prompt 都表示尚未选择默认值。"],
    [/^选择 \{\{name\}\}$/i, "选择技能"],
    [/^已捆绑$/i, "已安装"],
