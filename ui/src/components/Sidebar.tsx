    ...
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 pointer-coarse:gap-3 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          ...
          {(() => {
            const newTaskButton = (
              <button
                ...
                aria-label={rail ? t("nav.newTask", { defaultValue: "New Task" }) : undefined}
              ...
              >
                ...
                <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : "truncate"}>{t("nav.newTask", { defaultValue: "New Task" })}</span>
              </button>
            );
            return rail ? (
              <Tooltip>
                <TooltipTrigger asChild>{newTaskButton}</TooltipTrigger>
                <TooltipContent side="right">{t("nav.newTask", { defaultValue: "New Task" })}</TooltipContent>
              </Tooltip>
            ) : (
              newTaskButton
            );
          ...
          <SidebarNavItem to="/search" label={t("nav.search", { defaultValue: "Search" })} icon={Search} />
